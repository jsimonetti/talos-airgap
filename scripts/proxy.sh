    ...
    yq w -i join.yaml 'machine.files[1].permissions' '0644'
    yq w -i join.yaml 'machine.files[1].path' '/etc/ssl/certs/ca-certificates'
    yq w -i join.yaml 'machine.files[1].op' 'append'

    yq w -i -- controlplane.yaml 'machine.files[+].content' "$(cat ./certs/mitmproxy-ca-cert.cer)"
    yq w -i controlplane.yaml 'machine.files[1].permissions' '0644'
    yq w -i controlplane.yaml 'machine.files[1].path' '/etc/ssl/certs/ca-certificates'
    yq w -i controlplane.yaml 'machine.files[1].op' 'append'

    yq w -i -- init.yaml 'machine.files[+].content' "$(cat ./certs/mitmproxy-ca-cert.cer)"
    yq w -i init.yaml 'machine.files[1].permissions' '0644'
    yq w -i init.yaml 'machine.files[1].path' '/etc/ssl/certs/ca-certificates'
    yq w -i init.yaml 'machine.files[1].op' 'append'
